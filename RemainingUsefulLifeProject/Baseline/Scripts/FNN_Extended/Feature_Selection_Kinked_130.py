...
def print_results(model, x_test, y_test, tested_on, trained_on):
    ...
    plot_predicted_v_true(y_test, y_predicted, "Accuracy {}".format(tested_on),
                          "Extended FNN\\Feature Selection + Kinked 130\\Trained on {}".format(trained_on), show=False)
    plot_difference(y_test, y_predicted, 'Difference distribution {}'.format(tested_on),
                    "Extended FNN\\Feature Selection + Kinked 130\\Trained on {}".format(trained_on), show=False)
    print_scores(y_test, y_predicted, tested_on, "Extended FNN\\Feature Selection + Kinked 130\\Trained on {}".format(trained_on), tested_on)
